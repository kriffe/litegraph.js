...
*/
  ...
  const NODE_TITLE_HEIGHT = 16
  const NODE_SLOT_HEIGHT = 15
  const NODE_WIDTH = 140
  const NODE_MIN_WIDTH = 50
  const NODE_COLLAPSED_RADIUS = 10
  const NODE_COLLAPSED_WIDTH = 80
  const CANVAS_GRID_SIZE = 10
  const NODE_TITLE_COLOR = '#222'
  const NODE_DEFAULT_COLOR = '#999'
  const NODE_DEFAULT_BGCOLOR = '#444'
  const NODE_DEFAULT_BOXCOLOR = '#AEF'
  const NODE_DEFAULT_SHAPE = 'box'
  const MAX_NUMBER_OF_NODES = 1000 // avoid infinite loops
  const DEFAULT_POSITION = [100, 100] // default node position
  const node_images_path = ''
...
*/
  ...
  var createNode = function (type, title, options) {
    ...
    if (!node.pos) node.pos = DEFAULT_POSITION.concat()
    if (!node.mode) node.mode = ALWAYS
...
// separated just to improve if it doesnt work
  ...
  var isValidConnection = function (type_a, type_b) {
    // generic output
    if (!type_a || !type_b || type_a == type_a || (type_a !== EVENT && type_b !== EVENT && type_a.toLowerCase() == type_b.toLowerCase())) {
      return true
    }
    return false
  }
  ...
  if (typeof (performance) !== 'undefined') { getTime = function getTime () { return performance.now() } }
...
// iterations
      for (var i = 0; i < num; i++) {
        for (var j = 0, l = nodes.length; j < l; ++j) {
          var node = nodes[j]
          if (node.mode == ALWAYS && node.onExecute) { node.onExecute() }
        }
...
// Search for corner
        if (!skip_action && isInsideRectangle(e.canvasX, e.canvasY, n.pos[0], n.pos[1] - NODE_TITLE_HEIGHT, NODE_TITLE_HEIGHT, NODE_TITLE_HEIGHT)) {
          n.collapse()
          skip_action = true
        }
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__litegraph_js__ = __webpack_require__(0);
