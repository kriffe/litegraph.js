...
function multiConnection () {
  ...
  node_const_A.connect(0, node_watch, 0)
...
function sortTest () {
  var rand = createNode('math/rand', null, {pos: [10, 100] })
  graph.add(rand)

  var nodes = []
  for (var i = 4; i >= 1; i--)	{
    var n = createNode('basic/watch', null, {pos: [i * 120, 100] })
    graph.add(n)
    nodes[i - 1] = n
  }
...
//     var n = createNode('math/rand', null, {pos: [(2000 * Math.random()) | 0, (2000 * Math.random()) | 0] })
//     graph.add(n)
//     consts.push(n)
//   }
...
//   var watches = []
//   for (var i = 0; i < num_nodes; i++)	{
//     var n = createNode('basic/watch', null, {pos: [(2000 * Math.random()) | 0, (2000 * Math.random()) | 0] })
//     graph.add(n)
//     watches.push(n)
//   }

//   for (var i = 0; i < num_nodes; i++) { consts[ (Math.random() * consts.length) | 0 ].connect(0, watches[ (Math.random() * watches.length) | 0 ], 0) }
// }
